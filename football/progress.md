...
## 2026-09-09 — Football v1.31.0 adaptive challenge release
...
## 2026-09-18 — Football v1.33.0 (#132, superseding release entry)

- Added the shared DOM-free Grade 1-B (2026) curriculum authority in `shared/curriculum.js`: the full lesson catalog through printed page 187, page-gated operand rules for every arithmetic family, the packaged instructor-confirmed page-113 baseline, and progress-record validation. `shared/curriculum-ui.js` asks "What is the last page you finished?" once per session boundary and stores the learner-reported page browser-locally under `math-curriculum:progress:v1`.
- Football now confirms the completed page before every Quick Game or Season game, before `initGameSession` or any Season mutation; cancel returns to the start overlay with no side effects. Grade 1-B contextual arithmetic is completion-only and gated by `MATH_CURRICULUM.operationPage`; the packaged page-149 inclusion ceiling and the inherited 179-page validation bound no longer cap Grade 1-B. Grade 1-A, place-value mode, and Time Lab keep their existing boundaries.
- Place by Place gained a Whole book submode with original shapes, solids, fractions, measurement, graphs, and coins practice, plus page-gated mixed arithmetic and fact practice; its evidence lives in a separate `place-value-practice:book:v1` store. The registry gate accepts the exact two-target release `football,place-value-practice`.
- Synchronized `GAME_VERSION`, all Football asset cache keys, the Football and Place by Place registry descriptors, and `version.json` to 1.33.0 and 1.8.0; Kayak and Prague are unchanged.
- The Codex implementation was completed by Claude: `tests/football-context-integration.spec.mjs` now polls for the asynchronous curriculum prompt after Start instead of reading `learningState()` synchronously.
- Implementer verification passed the complete Football release gate (158 domain checks, 366 browser passes, 624 intentional device skips, 304 verified screenshots across six projects after removing only macOS-created empty duplicate device directories) and the Place release gate (45 domain checks, 394 browser passes, 20 skips), with the registry gate run as `REGISTRY_RELEASE_TARGET=football,place-value-practice`. The bundled client rendered both start prompts and the Whole book shapes question at 1180 x 820. This is emulated browser verification, not a physical-device run.

### Issue #132 review corrections

- Five-seat initial review of `7b30b6d` (Codex gpt-6-astra, Claude Fable 5.1, Grok 4.6, Gemini 3.1 Pro, Kimi K3). Verified findings and their fixes: three-addend tuples without a pair making ten now belong to printed page 124 rather than the page-103 doubles lesson, so a page-113 session cannot draw them; the Place by Place fact and Whole book lanes now own the page confirmed at their own restart, so a presentation or submode change cannot restore a stale parent page; the shared progress dialog is explicitly centered because `shared/reset.css` strips `dialog` margins; a fact with no resolvable source page is never eligible; every page-113 fallback reads `MATH_CURRICULUM.BASELINE`; the empty-state copy names the Whole book button.
- Added tests: exact three-addend and subtraction boundary pages plus a 120-question page-113 sweep; every mixed family reachable at page 187 and no family mapping to Infinity; Football contextual arithmetic declining `23 + 14`, `14 − 7`, and `67 − 24` with `curriculum-not-completed` at page 113 and unlocking them at 187 with the shared curriculum loaded; a primary-iPad browser check that a fact-focus restart to page 113 survives presentation and submode changes and that the prompt is centered.
- Recorded as follow-ups, not code changes: the app-start prompt is not reused when the app opens in Place value mode and every Whole book chapter change re-prompts (per the issue's session definition, but several prompts in quick succession); an unconfigured Place domain page (`null`) still means ungated for legacy callers, while every production path configures before creating or repairing a session; Football falls back to the packaged baseline rather than the persisted record on any path that bypasses `startGame`, which is conservative and only reachable through test boot modes.

## 2026-09-20 — Issue #109 release-artifact conflict copies (tooling only)

- Identified the likely producer of the empty ` 2`/` 3` device directories. The repository lives under iCloud Drive's synced `~/Documents`; the stray directories carried mode `0700` and a birth time from the previous run's pre-test step, ten minutes before the clean directories of the run that failed. The test process runs with umask `022` and created the clean directories as `0755` in the same run, and a fresh `mkdir` cannot carry an older birth time, so the strays were the previous run's directories restored and renamed after removal. Direct producer evidence (`fs_usage`) was not captured, so the sync daemon is the suspected, not proven, restorer.
- Moved the whole test artifact root to `tests/artifacts.nosync/`, which iCloud Drive excludes by name. Updated the Playwright `outputDir`, the pre/post release scripts, the release-matrix and arithmetic UI specs, `.gitignore`, and the manual-test notes. The verifier keeps its exact-directory and exact-screenshot requirements and now names ` N`-suffixed directories as suspected sync conflict copies in its error instead of deleting them.
- No runtime, question, version, or registry change; `GAME_VERSION` stays 1.33.0.
- Verification: `npm run test:football:release` exited zero unchanged with 161 DOM-free checks, 367 browser passes, 629 intentional device skips, and strict verification of all 304 screenshots across six projects, with no conflict copies under the new root. An earlier run of the same command under a host load average above 170 from unrelated sessions produced 13 timeout/browser-closed failures and still created no conflict copies. Registry gate 4/4 against `origin/main`.

### Issue #109 review corrections

- Codex (`gpt-6-astra`, medium) review of `673de93` requested two changes: retain the legacy `tests/artifacts/` ignore rule so stale screenshots in other checkouts stay unstaged, and qualify the root-cause attribution because `fs.mkdir` can yield `0700` under umask `077`. Both applied; the verifier hint and the notes now say suspected sync conflict copies. Registry gate rerun 4/4; the release gate is unaffected because no test or script path changed.

## 2026-09-20 — Football v1.33.1 (#128, superseding release entry)

- Extended the existing coarse-pointer landscape treatment from 1080px down to 1024px without changing portrait or orientation-agnostic tablet rules. The 1024–1079px landscape tier uses a smaller noninteractive field and compact two-column call, decision, question, feedback, and explanation layouts.
- Added deterministic 1024×688 coverage inside the existing iPad 11 landscape project. It verifies five-card offense and four-card defense calls, three-card fourth-down and two-card conversion decisions, question and feedback visibility with at least 16 CSS pixels of lower clearance, 44-point targets, zero automatic scroll, no horizontal overflow, and the two-column explanation state.
- Synchronized `GAME_VERSION`, all 18 Football asset query strings, the Football registry descriptor, and `version.json` to `1.33.1`; unrelated games remain unchanged.
- Physical Safari on an older 1024px-wide iPad remains unverified. The automated gate uses repository browser emulation and does not claim real-device proof.
- Verification passed the focused layout suite (21 passed, 15 intentional project skips), the bundled web-game client with semantic question state and no browser errors, the canonical release gate (161 DOM-free checks, 369 browser passes, 639 intentional device skips, and all 304 required screenshots across six projects), and the registry gate (4/4) against exact base `4e87ab4c868f456c1fe5aad36e1ca66ee05f20cd` with target `football`. Final 1024×688 question and feedback screenshots were visually inspected; both held `scrollY = 0`, a 688px document/viewport, and at least 17 CSS pixels below the desk.
