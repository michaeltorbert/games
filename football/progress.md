...
- Identified the producer of the empty ` 2`/` 3` device directories. The repository lives under iCloud Drive's synced `~/Documents`; the stray directories carried mode `0700` and a birth time from the previous run's pre-test step, ten minutes before the clean directories of the run that failed, so they were the removed previous-run directories resurrected by the sync daemon and renamed beside the fresh ones. `fs.mkdir` cannot produce that mode or birth time, so Playwright is not the source.
- Moved the whole test artifact root to `tests/artifacts.nosync/`, which iCloud Drive excludes by name. Updated the Playwright `outputDir`, the pre/post release scripts, the release-matrix and arithmetic UI specs, `.gitignore`, and the manual-test notes. The verifier keeps its exact-directory and exact-screenshot requirements and now names ` N`-suffixed directories as sync conflict copies in its error instead of deleting them.
